...
<article class="page-container">
	<form class="form form-horizontal" id="form-member-add">
        <input  type="text" name="playId" value="<%=plays.getPlayId()%>"/>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>充值金额：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="number" class="input-text" value="" placeholder="" id="money" name="money">
			</div>
		</div>
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
				<c:if test="<%=plays!=null%>">
					<input class="btn btn-primary radius" type="submit" value="&nbsp;&nbsp;提交&nbsp;&nbsp;">
				</c:if>
				<c:if test="<%=plays==null%>">
					<input class="btn btn-primary radius" type="button" onclick="login();" value="&nbsp;&nbsp;你暂未登录&nbsp;&nbsp;">
				</c:if>
			</div>
		</div>
	</form>
    ...
    $(function(){
        ...
        $("#form-member-add").validate({
            ...
            submitHandler:function(form){
                //$(form).ajaxSubmit();
				$.post("<%=path%>/money/addSaveMoney",$("#form-member-add").serialize(),
					function(data){
					},
					"json"
				);
