...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>道具管理 <span class="c-gray en">&gt;</span> 资讯列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" onclick="refush();" id="patrentutl"><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
        <a href="javascript:;" onclick="del()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
        <a class="btn btn-primary radius" data-title="添加道具"  onclick="prop_add('添加道具','<%=path%>/prop/initAddProp','','410')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加道具</a></span>
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
            ...
            <tbody>
                <c:forEach items="${propList}" var="s">
                    <tr class="text-c">
                        <td><input type="checkbox" value="${s.propId}" name="id"></td>
                        <td>${s.propName}</td>
                        <td>${s.propPrice}</td>
                        <td>${s.propSkill}</td>
                        <td>
                            <c:if test="${s.propGenre==0}">
                                英雄
                            </c:if>
                            <c:if test="${s.propGenre==1}">
                                武器
                            </c:if>
                            <c:if test="${s.propGenre==2}">
                                装备
                            </c:if>
                        </td>
                        <td class="f-14 td-manage">
                            <a style="text-decoration:none" class="ml-5" onClick="prop_edit('道具编辑','<%=path%>/prop/findProp/${s.propId}','','410')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
                            <a style="text-decoration:none" class="ml-5" onClick="prop_del(this,'${s.propId}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
                        </td>
                    </tr>
                </c:forEach>
            </tbody>
...
<script>
    ...
    function listrefush(){
        location.href="<%=path%>/prop/propList";
    }
</script>
