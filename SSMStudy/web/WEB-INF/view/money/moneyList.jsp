...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>金额管理 <span class="c-gray en">&gt;</span> 资讯列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" onclick="refush();" id="patrentutl"><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
        <a href="javascript:;" onclick="del()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
        <a class="btn btn-primary radius" data-title="添加金额"  onclick="money_add('添加金额','<%=path%>/money/initAddMoney','','410')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加金额</a></span>
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
            ...
            <tbody>
                <c:forEach items="${moneyList}" var="s">
                    <tr class="text-c">
                        <td><input type="checkbox" value="${s.moneyId}" name="id"></td>
                        <td>${s.moneyId}</td>
                        <td>${s.money}</td>
                        <td>${s.plays.playNickname}</td>
                        <td class="f-14 td-manage">
                            <a style="text-decoration:none" class="ml-5" onClick="money_edit('金额编辑','<%=path%>/money/findMoney/${s.moneyId}','','410')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
                            <a style="text-decoration:none" class="ml-5" onClick="money_del(this,'${s.moneyId}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
                        </td>
                    </tr>
                </c:forEach>
            </tbody>
...
<script>
    ...
    function listrefush(){
        location.href="<%=path%>/money/moneyList/<%=plays.getPlayId()%>";
    }
</script>
<script type="text/javascript">
    ...
    function money_edit(title,url,w,h){
    ...
    /*金额-删除*/
    function money_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.ajax({
                type: 'POST',
                url: '<%=path%>/money/deleteMoney/'+id,
                dataType: 'json',
