		...
		if(e.getSource()==shop){
			...
			try {
				...
				if(!vipcard.getText().equals("")){
					int num = (Integer.parseInt(sumshopping)-Integer.parseInt(shopnumber));
					if(num<0){
						string = shopnum+","+shopname+","+price+","+0+","+vipcard.getText()+","+new Date().toLocaleString()+";";
					}else {
						string = shopnum+","+shopname+","+price+","+num+","+vipcard.getText()+","+new Date().toLocaleString()+";";
					}
				}else {
					//非会员购买获取了一张会员卡
					if(sum>=200 && vipcard.getText().equals("")){
