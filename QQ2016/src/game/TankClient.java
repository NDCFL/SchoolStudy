...
//设置界面可以拖动的方法
	...
	private void setDragable() {
		this.addMouseListener(new java.awt.event.MouseAdapter() {
			public void mouseReleased(java.awt.event.MouseEvent e) {
				isDragged = false;
				TankClient.this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
			}
		...
		this.addMouseMotionListener(new java.awt.event.MouseMotionAdapter() {
			public void mouseDragged(java.awt.event.MouseEvent e) {
				if (isDragged) {
					loc = new Point(TankClient.this.getLocation().x + e.getX() - tmp.x,
							TankClient.this.getLocation().y + e.getY() - tmp.y);
					TankClient.this.setLocation(loc);
				}
			}
		});
	}
	...
	public TankClient() {
		...
		this.setTitle("坦克大战——(重新开始：R键  开火：F键)                 姓名：陈飞龙            班级：2016秋宏图一班" );
