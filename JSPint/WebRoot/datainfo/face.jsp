...
<div class="ConsoleLeft fl">
    ...
    <div class="ConsoleLeft_Tag_p">
        <p>IT课堂旗下IT技能学习平台，依托于11年行业品牌、1400万技术用户，提供辅导式“微职位”和自学式“视频课程”两种在线学习模式，帮助广大IT技术人员实现技能提升、高薪就业的职业梦想。</p>
    </div>
    ...
    <div class="ConsoleLeft_Tag_p">
        <p>IT课堂是旗下的职业IT在线教育平台，依托于11年行业品牌、1400万技术用户，目前已成为中国最大的IT技能学习平台。IT课堂秉承“学以致用”、 “因材施教”、“有教无类”的教育理念，提供辅导式“微职位”和自学式“视频课程”两种在线学习模式，帮助广大IT技术人员实现技能提升、高薪就业的职业梦想。</p>
    </div>
    <div class="ConsoleLeft_Tag">标签</div>
    <div class="ConsoleLeft_Tag_p">
                 <a href="<%=basePath %>courselistservlet?order=datainfo&sectionid=${courseBean1.sectionid}&courseid=${courseBean1.course_id}" target="_blank"  style="white-space:nowrap;margin:3px;">${courseBean1.course_name}</a>
                <a href="<%=basePath %>courselistservlet?order=datainfo&sectionid=${courseBean2.sectionid}&courseid=${courseBean2.course_id}" target="_blank"  style="white-space:nowrap;margin:3px;">${courseBean2.course_name}</a>
                <a href="<%=basePath %>courselistservlet?order=datainfo&sectionid=${courseBean3.sectionid}&courseid=${courseBean3.course_id}" target="_blank"  style="white-space:nowrap;margin:3px;">${courseBean3.course_name}</a>
                <a href="<%=basePath %>courselistservlet?order=datainfo&sectionid=${courseBean4.sectionid}&courseid=${courseBean4.course_id}" target="_blank"  style="white-space:nowrap;margin:3px;">${courseBean4.course_name}</a>
                <a href="<%=basePath %>courselistservlet?order=datainfo&sectionid=${courseBean5.sectionid}&courseid=${courseBean5.course_id}" target="_blank"  style="white-space:nowrap;margin:3px;">${courseBean5.course_name}</a>
                <a href="<%=basePath %>courselistservlet?order=datainfo&sectionid=${courseBean1.sectionid}&courseid=${courseBean1.course_id}" target="_blank"  style="white-space:nowrap;margin:3px;">${courseBean1.course_name}</a>
                <a href="<%=basePath %>courselistservlet?order=datainfo&sectionid=${courseBean2.sectionid}&courseid=${courseBean2.course_id}" target="_blank"  style="white-space:nowrap;margin:3px;">${courseBean2.course_name}</a>
                <a href="<%=basePath %>courselistservlet?order=datainfo&sectionid=${courseBean3.sectionid}&courseid=${courseBean3.course_id}" target="_blank"  style="white-space:nowrap;margin:3px;">${courseBean3.course_name}</a>
                <a href="<%=basePath %>courselistservlet?order=datainfo&sectionid=${courseBean4.sectionid}&courseid=${courseBean4.course_id}" target="_blank"  style="white-space:nowrap;margin:3px;">${courseBean4.course_name}</a>
          		...
          		<c:forEach items="${courselist}" var="l">
          			...
          			<div class="p_kc_li clearfix bd_bottom1" style="position: relative;"> 
	            		<a href="/course/course_id-7737.html" target="_blank">
	             			<img alt="点击进入详情" src="${l.picture_url}" width="126" height="94" />
	            		</a>
	            		<dl>
		              		<dt class="font14">
		                  		<span class="fl kc_name"><a href="<%=basePath %>datainfo/courseinfo.jsp?courseid=${l.course_id}" target="_blank">${l.course_name}----${l.one_word}(${l.course_count}课时)</a></span>
		              	  		<span class="fr c_red font12 tr">
		              		...
		              		<dd class="p_dd03">
			                	<p class="fl p_dd03_l">
									...
									<span><em><a href="" target="_blank">0</a></em>条讨论</span>
									<c:if test="${l.ismoney==1}">
		              	  				<span><font color="red"><em>￥</em>${l.price}</font></span>
		              	  			</c:if>
								</p>
			                	<span class="p_play">
			                    	<a href="<%=basePath %>courselistservlet?order=datainfo&courseid=${l.course_id}" target="_blank"><img alt="开始学习" src="https://static1.51cto.com/edu/images/p_play_ico.png" align="absmiddle" />开始学习</a>
			                	</span>
	              	  		</dd>
            			</dl>
          			</div>
          		</c:forEach>
          		<div class="box3"><div class="aa"><div class="pagesDevice"><a href="<%=basePath %>courselistservlet?order=shousuo&currenpage=1&handle=firstpage&sousuo=<%=request.getAttribute("name") %>" class="pagesNum pagesNumCur">1</a><a href="<%=basePath %>courselistservlet?order=shousuo&currenpage=2&handle=downpage&sousuo=<%=request.getAttribute("name") %>" class="pagesNum">2</a><a href="<%=basePath %>courselistservlet?order=shousuo&currenpage=${pager.currentpage }&handle=downpage&sousuo=<%=request.getAttribute("name") %>" class="pagesGoNext">&nbsp;</a></div></div></div>
          </td>
      </tr>
    </table>
  </div>
...
<script>
    ...
    $('p[name=coupon_btn]').one('click',function(){
        ...
        if(user_id == '' || user_id == '0') {
          window.location.href='http://home.51cto.com/index/?reback=http%253A%252F%252Fedu.51cto.com%252Flecturer%252Fuser_id-8899635.html';
          return false;
        }
        var me = $(this);
        var meid = me.attr('id');
...
<div class="Footer">
    <div class="Page">
        <table class="QRcode tc">
            <caption>
                <img src="<%=basePath %>image/itlogo.png" alt="IT课堂" width="194" height="40"/>
            </caption>
            <tr>
                <td>
...
<div class="Advice disp-n" id="Advice">
    <div class="Main">
        ...
        <h3>可以开发票么？</h3>
        <p>可以的，购买课程或者充值余额后都是可以开具发票的，具体详情点击：<a href="<%=basePath %>courselistservlet?order=list" target="_blank">我要开发票</a></p>
        <h3>购买的课程可以下载么？</h3>
