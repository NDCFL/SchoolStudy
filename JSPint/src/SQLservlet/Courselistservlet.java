	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(order.equals("list")){
			...
			if(studentBean==null){
				...
				List<CourseBean> courselist = sql.courselist(6, 0);
				request.setAttribute("courselist", courselist);
				int num = sql.barsum("select count(*) cnt from shoppingcar where studentname='"+studentBean.getStudentname()+"'");
...
//				sql.courseupdate(((int)coursebean.getPrice())-((int)codeBean.getExchangemeony()), Integer.parseInt(courseid));
				...
				request.setAttribute("price",price );
				request.setAttribute("courseprice",((int)coursebean.getPrice())-((int)codeBean.getExchangemeony()));
				TeacherBean teacherbean = sql.findteacher(coursebean.getTeacher_id()+"");
				request.setAttribute("teacherbean", teacherbean);
				String codeid = request.getParameter("code");
				ExchangeCodeBean codebean = sql.findexcodebean(codeid);
				...
				int num = sql.barsum("select count(*) cnt from shoppingcar where studentname='"+studentBean.getStudentname()+"'");
				request.setAttribute("num", num);
				request.setAttribute("courseprice", ((coursebean.getPrice()-codeBean.getExchangemeony())*price/10));
				request.getRequestDispatcher("datainfo/zhifuzhongxin.jsp").forward(request, response);
			}
			...
			CourseBean teacher = sql.findcoursemanger(request.getParameter("courseid"));
			float qian = 0;
			...
			ShoppingcarBean shopcar1 =sql.findshopcar(courseid); 
			if(shopcar1.getStudentname()!=null){
				...
				shopcar.setStudentname(studentBean.getStudentname());
			...
			int num=0;
			if(studentBean!=null){
				num = sql.barsum("select count(*) cnt from shoppingcar where studentname='"+studentBean.getStudentname()+"'");
			}
			request.setAttribute("num", num);
	...
	public void paginglist1(HttpServletRequest request,
		...
		List<ShoppingcarBean> shoplist = sql.shopcarlist(page.getPagebarsize(), page.getStarlocal(),studentBean.getStudentname());
	...
	public void paginglist3(HttpServletRequest request,
		...
		List<ForumBean> forlist = sql.liuyanlist(page.getPagebarsize(), page.getStarlocal(), replayid);
		request.setAttribute("forlist", forlist);
		request.setAttribute("pager", page);
	}
