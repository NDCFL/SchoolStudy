	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("order") != null) {
			...
			if (order.equals("list")) {
				...
				List<BankBean> banklist = sql.banklist();
				request.setAttribute("banklist", banklist);
				request.getRequestDispatcher("data/teacherupdate.jsp").forward(
						request, response);
				...
				try {
					...
					teachername = mr.getParameter("teachername");
					...
					sql.updateface("teacher", "teacherpictureurl", "upfile/"+fileName, "teachername", new String(teachername.getBytes("iso-8859-1"),"utf-8"));
					sql.updateface("student", "studentface", "upfile/"+fileName, "studentname", new String(teachername.getBytes("iso-8859-1"),"utf-8"));
					paginglist(request, response);
			...
			}else if(order.equals("checkstudentface")){
				MultipartRequest mr = null;
				String path = getServletContext().getRealPath("/")+"upfile"; 
				File file = new File(path);
				if (!file.exists()) {
				...
				try {
					mr = new MultipartRequest(request,path,fileupsize,"utf-8",filename);
					teachername = mr.getParameter("studentname");
					Enumeration filesname = mr.getFileNames();   
			        while (filesname.hasMoreElements()) {  
			            String name = (String) filesname.nextElement();  
			            fileName = mr.getFilesystemName(name); 
