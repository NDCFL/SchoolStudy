...
public class payrecodeservlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("order") != null) {
			...
			if (order.equals("delete")) {
				...
				StudentBean student = sql.findstudent(name);
	...
	public void paginglist(HttpServletRequest request,
		...
		// 总记录条数，计算总页数
		page.setPagebarsum(sql.barsum("select count(*) from payrecord p,course c,section s where p.courseid=c.course_id and  paystatus=1 and payrecordid not in (select top(0) payrecordid from payrecord where paystatus=1) and studentid="+studentBean.getStudentid()+" and p.courseid=s.courseid "));
		// 当前页
		...
		List<PayRecordBean> paylist = sql.payrecodelist(10, page.getStarlocal(), "1",studentBean.getStudentid());
